...
public class SqlController {
    ...
    private UserDetailsResponse login(@PathVariable String userName, @PathVariable String pwd) throws SQLDataException {

        UserDetailsResponse us=new UserDetailsResponse();
        UserDetail use= userRepo.findByUserNameAndPassword(userName,pwd);
        if(use!= null){
            us.setStatus("true");
